...
class NextflowConfig:
    ...
    def dump_to_file(self):
        ...
        #f.write("process.memory = { 8.GB * task.attempt }\n")
        f.write("process.memory = { 16.GB * task.attempt }\n")
        ...
        f.write(f"process.maxRetries = 10\n")
        #f.write("process.errorStrategy = { task.exitStatus == 140 ? 'retry' : 'ignore' }\n")
        #f.write("process.errorStrategy = { task.exitStatus == 140 ? 'retry' : 'retry' }\n")
        #f.write("process.errorStrategy = { sleep(Math.pow(2, task.attempt) * 200 as long); return 'retry' }\n")
